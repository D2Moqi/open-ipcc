...
## 更新摘要
**变更内容**
- 更新了技术栈版本至 Java 17 + Spring Boot 3.5、Vue 3.5 + Vite 8
- 增强了呼叫中心模块的架构描述和组件说明
- 改进了开发规范和工具链配置指导
- 更新了版本控制配置，新增对 .workbuddy 目录的忽略规则
- 完善了扩展开发方法和故障排查指南
...
## 简介
本指南面向呼叫中心（CC）二次开发团队，基于 yudao-cloud 框架进行后端、前端与产品文档的协同开发。项目采用单体启动模式，核心业务集中在 yudao-module-cc；前端为 Vue 3.5 + Vite 8 + Element Plus + TypeScript；产品需求文档与原型以纯 HTML 形式维护。本文档覆盖开发规范、提交与分支策略、扩展开发方法（自定义处理器/插件/第三方集成）、工具链配置与调试、代码审查标准、质量保证流程、持续集成建议、新功能开发与版本升级注意事项，以及团队协作最佳实践。
...
### 后端模块与启动装配
- 启动入口与模块聚合
  - 顶层 pom.xml 定义模块列表与统一版本管理，当前启用 system、infra、cc。
...
## 结论
本项目以 yudao-cloud 为基础，聚焦呼叫中心场景，采用单体启动与多模块架构，前后端分离，强调代码质量与可维护性。通过严格的代码规范、自动化质量检查与清晰的扩展点设计，团队可高效开展新功能开发与系统集成。建议在迭代中持续完善测试覆盖率、性能基准与监控告警，保障系统稳定演进。
...
- `ipcc-sipproxy/.gitignore`：新增 Trae IDE 相关规则和 Python 相关忽略
...
### 版本升级注意事项
- 后端
  - 关注 Spring Boot 3.5、JDK 17、依赖版本升级带来的兼容性问题；检查注解处理器与编译参数。
- 前端
...
**章节来源**
- [pom.xml:39-53](file://yudao-cloud/pom.xml#L39-L53)
...
- **编译要求**：根 pom 已配置 Lombok + MapStruct 注解处理器与 -parameters 编译参数；JAIN-SIP 版本必须统一为 1.2.1.4，避免跨版本 AbstractMethodError。
...
#### 后端技术栈
- **Java 17**：最新 LTS 版本，提供更好的性能和内存管理
- **Spring Boot 3.5.15**：最新稳定版本，支持现代 Java 特性和云原生开发
- **Maven 多模块架构**：统一的依赖管理和构建流程
- **核心组件**：
  - ipcc-fs-esl：基于 Netty 4.x 的 FreeSWITCH ESL 客户端
