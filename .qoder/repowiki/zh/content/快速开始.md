...
- [yudao-cloud/yudao-dependencies/pom.xml](file://yudao-cloud/yudao-dependencies/pom.xml)
- [yudao-cloud/yudao-server/pom.xml](file://yudao-cloud/yudao-server/pom.xml)
</cite>

## 更新摘要
**变更内容**

- 升级至 JDK 25 + Spring Boot 4.1 技术栈
- 更新 Maven 构建系统配置，增强插件执行跳过配置以提升构建效率
- 调整依赖管理版本，包括 Spring Cloud 2025.1 和相关组件
- 优化编译参数和注解处理器配置以支持新版本的 Java 编译器
- 更新前端技术栈版本信息
...
本项目是基于yudao-cloud二次开发的呼叫中心系统，采用最新的 **JDK 25 + Spring Boot 4.1** 技术栈，由三个独立Git仓库（git
submodule）组成，各自独立提交与分支。
...
| 目录                   | 说明                                                 | 技术栈                                              |
|------------------------|------------------------------------------------------|-----------------------------------------------------|
| `yudao-cloud/`         | 后端微服务（含自定义呼叫中心模块 `yudao-module-cc`） | JDK 25 + Spring Boot 4.1 + Maven 多模块             |
...
- **IntelliJ IDEA**：推荐的后端IDE（需支持JDK 25）
- **VS Code**：推荐的前端编辑器

**更新** 项目现已升级至JDK 25和Spring Boot 4.1，需要确保开发环境使用最新版本的JDK。

**章节来源**

- [yudao-cloud/pom.xml:42-46](file://yudao-cloud/pom.xml#L42-L46)
- [yudao-cloud/yudao-dependencies/pom.xml:20-22](file://yudao-cloud/yudao-dependencies/pom.xml#L20-L22)
...
- **JDK 25**（已升级）
- **Spring Boot 4.1.0**（已升级）
- **Spring Cloud 2025.1.2**（已升级）
- **Maven多模块架构**
...
participant BE as "后端(Spring Boot 4.1)"
...
- JDK 25+ 默认禁用 classpath 隐式注解处理，必须显式声明 `proc=full`
- JAIN-SIP版本必须统一为1.2.1.4，避免跨版本AbstractMethodError

### 提交注意
- 三个子仓库各自独立提交推送，需在对应目录内执行git操作

**更新** 由于升级到JDK 25，编译时需要特别注意注解处理器的配置，已自动配置为`proc=full`以解决JDK 22+的默认行为变化。

**章节来源**
- [AGENTS.md:73-79](file://AGENTS.md#L73-L79)
- [yudao-cloud/pom.xml:110-112](file://yudao-cloud/pom.xml#L110-L112)
...
- 确认端口未被占用
- 查看日志文件定位具体错误
- **更新** 确保使用JDK 25，旧版本JDK可能不兼容Spring Boot 4.1
...
- **更新** 如果遇到构建性能问题，可使用`-DskipTests`跳过测试以提高构建速度
- 检查Maven版本兼容性
- 确认所有依赖版本与Spring Boot 4.1兼容
...
**重要更新** 本项目现已全面升级至JDK 25 + Spring Boot
4.1技术栈，提供了更好的性能和安全性。构建系统经过优化，支持更高效的插件执行和依赖管理。本系统提供了完整的呼叫中心解决方案，包括实时通信、IVR流程、坐席管理等核心功能。通过合理的架构设计和模块化组织，确保了系统的可扩展性和可维护性。